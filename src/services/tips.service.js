...
async function addTip({ user, id, content, title, date, label }) {
    console.log('%c  mail, id, content, title, date:', 'color: white;background: red;', JSON.stringify({ mail: user.mail, id, content, title, date, label }));
...
async function removeTip(tipId) {
    console.log('%c  storyId', 'color: white;background: red;', JSON.stringify({ id: tipId }));
